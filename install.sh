...
if ! python3 -c "import redis" 2>/dev/null; then
    info "安装 python3-redis..."
    apt-get install -y python3-redis 2>/dev/null \
    || pip3 install redis --break-system-packages -q 2>/dev/null \
...
echo "  redis-export.py        - 导出/推送 Redis 数据"
echo ""
echo "示例:"
echo "  redis-export.py dump -a 密码"
echo "  redis-export.py push 目标IP 6379 目标密码 --flush"
